...
export async function GET(request: Request) {
  try {
    ...
    const dhanResponse = await fetch(`${baseUrl}/positions`, options);
    ...
    // If no positions or empty response, return { data: [] } with 200
    ...
    console.error("Error fetching positions from Dhan API:", error);
    return new Response(
      JSON.stringify({
