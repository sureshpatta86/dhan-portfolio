...
export async function GET(request: Request) {
  try {
    ...
    const dhanResponse = await fetch(`${baseUrl}/fundlimit`, options);
    ...
    // If no funds data or empty response, return default structure
    if (!responseData) {
      const defaultFunds = {
        dhanClientId: "",
        availabelBalance: 0,
    ...
    console.error("Error fetching funds from Dhan API:", error);
    return new Response(
      JSON.stringify({
