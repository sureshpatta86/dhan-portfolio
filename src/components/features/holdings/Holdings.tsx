...
export default function Holdings() {
  ...
  const handleOrderPlaced = () => {
    // Refresh holdings data after order placement
    refetch();
  };
  ...
  const processedHoldings = (data || [])
    .filter((holding: DhanHolding) =>
    ...
    .sort((a: DhanHolding, b: DhanHolding) => {
      switch (sortBy) {
        case 'name':
          return a.tradingSymbol.localeCompare(b.tradingSymbol);
        case 'value':
          const valueA = a.totalQty * a.avgCostPrice;
          const valueB = b.totalQty * b.avgCostPrice;
          return valueB - valueA; // Descending order
        case 'quantity':
          return b.totalQty - a.totalQty; // Descending order
        default:
          return 0;
      }
    });
  ...
  return (
    <div className="p-6">
      ...
      <HoldingsStats holdings={processedHoldings} />
      ...
      {searchTerm && (
        <div className="mb-4 text-sm text-gray-600">
          Found {processedHoldings.length} holding(s) matching "{searchTerm}"
        </div>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {processedHoldings.map((holding: DhanHolding) => (
          <HoldingsCard 
      ...
      {processedHoldings.length === 0 && searchTerm && (
